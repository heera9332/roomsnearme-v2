      ...
      <table className="w-full mb-4 border">
        <thead>
          <tr>
            ...
            <th className="p-2 bg-gray-50 text-left">Check-In</th>
            <th className="p-2 bg-gray-50 text-left">Check-Out</th>
            <th className="p-2 bg-gray-50 text-left">Price/Day</th>
            <th className="p-2 bg-gray-50 text-left">Days</th>
        ...
        <tbody>
          {items.map((item) => {
            const stateDate = dates[item.id] || {}
            const checkIn = stateDate.checkIn || formatDate(item.checkInDate)
            const checkOut = stateDate.checkOut || formatDate(item.checkOutDate)
            const days = getDaysDiff(checkIn, checkOut)
            return (
              <tr key={item.id} className="border-t">
                ...
                <td className="p-2">₹{perDayPrice(item)}</td>
                <td className="p-2">{days}</td>
                <td className="p-2 font-bold">₹{itemTotal(item)}</td>
                <td className="p-2">
                  <button
